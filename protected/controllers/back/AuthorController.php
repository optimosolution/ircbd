...
class AuthorController extends BackEndController {
    ...
    public function actionCreate() {
        $model = new Author;
        $path = Yii::app()->basePath . '/../uploads/author';
        if (!is_dir($path)) {
            mkdir($path);
        }
        ...
        if (isset($_POST['Author'])) {
            ...
            if ($model->validate()) {
                ...
                if (@!empty($_FILES['Author']['name']['author_photo'])) {
                    $model->author_photo = $_POST['Author']['author_photo'];

                    if ($model->validate(array('author_photo'))) {
                        $model->author_photo = CUploadedFile::getInstance($model, 'author_photo');
                    } else {
                        $model->author_photo = '';
                    }
                    $model->author_photo->saveAs($path . '/' . time() . '_' . str_replace(' ', '_', strtolower($model->author_photo)));
                    $model->author_photo = time() . '_' . str_replace(' ', '_', strtolower($model->author_photo));
                }
                if ($model->save()) {
    ...
    public function actionUpdate($id) {
        $model = $this->loadModel($id);
        $previuosFileName = $model->author_photo;
        $path = Yii::app()->basePath . '/../uploads/author';
        if (!is_dir($path)) {
            mkdir($path);
        }
        ...
        if (isset($_POST['Author'])) {
            ...
            if ($model->validate()) {
                ...
                if (@!empty($_FILES['Author']['name']['author_photo'])) {
                    ...
                    $model->author_photo = $previuosFileName;
                }
                if ($model->save()) {
                    Yii::app()->user->setFlash('success', 'Data was saved successfully');
                    $this->redirect(array('admin'));
