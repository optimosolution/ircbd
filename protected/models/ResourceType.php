...
class ResourceType extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('resource_type', 'length', 'max' => 150),
            // The following rule is used by search().
            // @todo Please remove those attributes that should not be searched.
            array('id, resource_type', 'safe', 'on' => 'search'),
        );
    }
    ...
    public static function get_title($id) {
        $value = ResourceType::model()->findByAttributes(array('id' => $id));
        if (empty($value->resource_type)) {
            return null;
        } else {
